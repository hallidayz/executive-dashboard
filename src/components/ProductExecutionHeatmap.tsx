...
export const ProductExecutionHeatmap: React.FC<
  ProductExecutionHeatmapProps
> = ({ products, onNavigateTab }) => {
      ...
      <div className="glass-panel p-5 rounded-2xl border brand-border bg-gradient-to-r from-obsidian-900 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Rocket className="w-5 h-5 brand-text" />
            <h3 className="font-bold text-slate-100 text-lg">
              Product Execution & Delivery Portfolio
            </h3>
          </div>
          <p className="text-xs text-slate-300">
            Real-time launch readiness, sprint velocity, and blocker tracking
            across all product lines you oversee as Head of Product.
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 shrink-0">
          <div className="glass-card p-3 rounded-xl border brand-border text-center">
            ...
            <p className="text-xl font-black brand-text">{avgReadiness}%</p>
          </div>
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-400">
                    Lead:{" "}
                    <strong className="text-slate-200">{prod.lead}</strong>
                  </p>
                </div>
                ...
                <div className="w-full h-2 rounded-full bg-obsidian-950 overflow-hidden border border-slate-800">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      prod.readinessScore >= 90
                        ? "bg-gradient-to-r from-emerald-500 to-[var(--status-success)]"
                        : prod.readinessScore >= 80
                          ? "brand-gradient"
              ...
              <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between text-xs">
                <div className="flex items-center gap-1.5 text-slate-300">
                  <Zap className="w-3.5 h-3.5 brand-text shrink-0" />
