...
import { Sparkles, Bot, Send, Minimize2, ArrowLeftRight } from "lucide-react";
...
export const FloatingAIAdvisorPanel: React.FC<FloatingAIAdvisorPanelProps> = ({
  ...
  const handleSendPrompt = (userText: string) => {
    ...
    setChatLog((prev) => [
      ...prev,
      { role: "user", text: userText, timestamp: time },
    ]);
    setAiPrompt("");
      ...
      <div className="glass-panel w-80 md:w-96 rounded-2xl brand-border brand-ring bg-obsidian-950/95 shadow-2xl flex flex-col h-[520px] overflow-hidden backdrop-blur-xl">
        <div className="p-3.5 border-b border-slate-800/80 brand-bg-soft flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              onClick={() =>
                setDockPosition(dockPosition === "right" ? "left" : "right")
              }
            ...
            >
              <ArrowLeftRight className="w-3.5 h-3.5 brand-text" />
            ...
            >
              <div
                className={`p-3 rounded-2xl max-w-[90%] leading-relaxed ${
                  msg.role === "user"
                    ? "brand-button rounded-br-none shadow-md"
        ...
        <div className="p-2.5 bg-obsidian-900/80 border-t border-slate-800/80 space-y-2">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
            {quickActions.map((qa, idx) => (
              <button
                ...
                className="px-2.5 py-1 rounded-lg brand-bg-soft hover:opacity-90 brand-text border brand-border text-[10px] font-semibold whitespace-nowrap transition-all shrink-0"
              >
                {qa.label}
              </button>
            ))}
          ...
          >
            ...
            <button
              type="submit"
              className="p-2 rounded-xl brand-button brand-ring transition-all shrink-0"
            >
