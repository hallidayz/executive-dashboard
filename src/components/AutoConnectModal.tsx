...
import { X, Cpu, Zap, Check, RefreshCw, Radio, Bot, Layers, Sparkles } from 'lucide-react';
...
export const MOCK_DISCOVERABLE_SYSTEMS: AutoConnectorSpec[] = [
  {
    ...
    description: 'Local n8n workflow orchestrator running 4 executive automation pipelines.',
    icon: 'Share2',
    autoConnect: true,
  },
  {
  ...
  },
  {
    ...
    status: 'Auto-Discovered',
    description: 'Local code synthesis & repository refactoring subagent system.',
    icon: 'Code',
    autoConnect: true,
  },
  {
  ...
  },
  {
    ...
    status: 'Auto-Discovered',
    description: 'Claude 3.7 Sonnet for executive document analysis and coding.',
    icon: 'Sparkles',
    autoConnect: true,
  },
  {
...
export const AutoConnectModal: React.FC<AutoConnectModalProps> = ({
  ...
  const [systems, setSystems] = useState<AutoConnectorSpec[]>(MOCK_DISCOVERABLE_SYSTEMS);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-obsidian-950/80 backdrop-blur-md">
      <div className="glass-panel w-full max-w-3xl rounded-2xl border border-indigo-500/40 p-6 space-y-5 shadow-2xl relative">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-100 text-lg">Universal Model & Agent Auto-Connect Screen</h3>
              <p className="text-xs text-slate-400">Auto-discovers local ports, ChatGPT, Claude, Gemini, Hermes, ClawCode, OpenClaw & n8n.</p>
            </div>
          </div>
        ...
        {/* Scan Bar */}
        <div className="p-3.5 rounded-xl glass-card border border-indigo-500/30 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-slate-200 font-medium">
            <Sparkles className="w-4 h-4 text-indigo-400" />
            <span>Scanning local machine ports (11434, 1234, 5678, 8080, 9090)...</span>
          </div>
          <button
            onClick={handleScan}
            disabled={isScanning}
            ...
            return (
              <div
                ...
                className={`p-4 rounded-xl glass-card border transition-all space-y-2 ${
                  isConn ? 'border-emerald-500/50 bg-emerald-950/20' : 'border-slate-800 hover:border-indigo-500/30'
                }`}
              >
                ...
                <code className="text-[10px] p-1.5 rounded bg-obsidian-950 text-indigo-300 block font-mono truncate">
                  {sys.endpoint}
                </code>
