...
export const ChiefOfStaffView: React.FC<ChiefOfStaffViewProps> = ({
  summary,
  onNavigateTab,
}) => {
  const [activeHorizon, setActiveHorizon] = useState<
    "today" | "tomorrow" | "week" | "beyond"
  >("today");
  ...
  const handleSendPrompt = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (
        ...
        userText.toLowerCase().includes("meeting")
      ) {
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl glass-panel p-6 border brand-border bg-gradient-to-r from-obsidian-900 brand-ring">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full brand-bg-soft brand-text border brand-border text-xs font-semibold">
            ...
            <p className="text-sm text-slate-300">
              Your AI Chief of Staff has synthesized your Outlook Calendar,
              Priority Emails, Notion Action Items, and Krisp Notes to maximize
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="glass-card p-3 rounded-xl border border-slate-800 text-center">
              ...
              <p className="text-xl font-extrabold brand-text">
                {summary.calendarDensityPercentage}%
              </p>
            </div>
            <div className="glass-card p-3 rounded-xl border border-slate-800 text-center">
              <span className="text-xs text-slate-400 font-medium">
        ...
        <div className="lg:col-span-2 glass-panel rounded-2xl p-5 space-y-4 border border-slate-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-slate-100 text-base">
                Top Strategic Directives Today
              </h3>
            </div>
          ...
          <div className="space-y-3">
            {summary.topDirectives.map((directive, idx) => (
              <div
                key={idx}
                className="glass-card p-4 rounded-xl flex items-start gap-3 border brand-border hover:border-[var(--brand-accent-border)]"
              >
                <div className="p-2 rounded-lg brand-bg-soft brand-text shrink-0 mt-0.5">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-400">
                    <span className="brand-text font-medium">Priority #1</span>
                    <span>•</span>
        ...
        <div className="glass-panel rounded-2xl p-5 space-y-4 border border-slate-800 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <TrendingUp className="w-5 h-5 brand-text" />
                <h3 className="font-bold text-slate-100 text-base">
                  Energy & Focus Density
                </h3>
              </div>
              <span className="text-xs brand-text font-semibold">
                Peak: 10:30 AM
              </span>
            </div>
            <p className="text-xs text-slate-400 mb-4">
              AI recommendation: Schedule deep strategic thinking during your
              10:30 AM focus window.
            </p>
          ...
          <div className="p-3 rounded-xl brand-bg-soft border brand-border text-xs brand-text flex items-center gap-2">
            <Clock className="w-4 h-4 shrink-0 brand-text" />
            <span>
              Next Deep Work Window: 10:30 AM - 12:00 PM (Local Laptop KB
              Training)
            </span>
          </div>
        </div>
      ...
      <div className="glass-panel rounded-2xl p-6 border border-slate-800 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
          <div className="flex items-center gap-2">
            <Layers className="w-5 h-5 brand-text-secondary" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
          ...
          {activeHorizon === "tomorrow" && (
            <div className="col-span-2 glass-card p-4 rounded-xl space-y-3 border brand-border">
              <div className="flex items-center gap-2 brand-text font-bold text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {summary.tomorrowLookahead.map((item, i) => (
                  <div
                    key={i}
                    className="p-3 rounded-lg bg-obsidian-950/60 border border-slate-800 text-xs text-slate-300"
                  >
                    <span className="brand-text font-bold block mb-1">
                      Step #{i + 1}
                    </span>
                    {item}
                  </div>
          ...
          {activeHorizon === "week" && (
            <div className="col-span-2 glass-card p-4 rounded-xl space-y-3 border border-purple-500/20">
              <div className="flex items-center gap-2 brand-text-secondary font-bold text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {summary.weekLookahead.map((item, i) => (
                  <div
                    ...
                    className="p-3 rounded-lg bg-obsidian-950/60 border border-slate-800 text-xs text-slate-300"
                  >
                    <span className="brand-text-secondary font-bold block mb-1">
                      Goal #{i + 1}
                    </span>
                    {item}
                  </div>
      ...
      <div className="glass-panel rounded-2xl p-5 border border-slate-800 space-y-4">
        <div className="flex items-center gap-2">
          <Bot className="w-5 h-5 brand-text" />
          <h3 className="font-bold text-slate-100 text-base">
            Chief of Staff Interactive AI Advisor
          </h3>
        </div>
        ...
        <div className="space-y-3 max-h-48 overflow-y-auto pr-2">
          {chatLog.map((msg, i) => (
            ...
            >
              {msg.role === "assistant" && (
                <div className="w-6 h-6 rounded-lg brand-bg-soft brand-text border brand-border flex items-center justify-center shrink-0">
        ...
        <form onSubmit={handleSendPrompt} className="flex gap-2">
          ...
          <button
            type="submit"
            className="px-4 py-2.5 rounded-xl brand-button font-medium text-xs flex items-center gap-1.5 brand-ring transition-all"
          >
            <span>Ask AI</span>
