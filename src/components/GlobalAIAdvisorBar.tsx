...
import { Sparkles, Bot, Send, Zap, ChevronDown, ChevronUp, Target } from 'lucide-react';
...
export const GlobalAIAdvisorBar: React.FC<GlobalAIAdvisorBarProps> = ({ summary, onNavigateTab }) => {
  ...
  const handleSendPrompt = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      } else if (userText.toLowerCase().includes('krisp') || userText.toLowerCase().includes('notion')) {
  ...
  return (
    <div className="glass-panel rounded-2xl border border-indigo-500/40 bg-gradient-to-r from-obsidian-900 via-indigo-950/70 to-purple-950/70 p-4 shadow-glow-indigo transition-all">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-indigo-500 to-purple-600 p-0.5 shrink-0 flex items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-bold text-xs text-white">Chief of Staff AI Advisor</span>
              <span className="px-2 py-0.5 text-[9px] font-bold rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 uppercase">
                Live Co-Pilot
              </span>
            </div>
            <p className="text-xs text-slate-300 truncate">{chatLog[chatLog.length - 1]?.text}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">{isExpanded ? 'Collapse' : 'Ask AI'}</span>
          </button>
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="mt-3 pt-3 border-t border-slate-800/80 space-y-3">
          <form onSubmit={handleSendPrompt} className="flex gap-2">
            ...
            <button
              type="submit"
              className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs flex items-center gap-1.5 shadow-md shadow-indigo-600/30 transition-all shrink-0"
            >
              <span>Ask AI</span>
