...
export const AppLauncher: React.FC<AppLauncherProps> = ({
  ...
  const [runningCustomApp, setRunningCustomApp] =
    useState<CustomAppSpec | null>(null);

  const [appName, setAppName] = useState("");
  const [appUrl, setAppUrl] = useState("");
  const [appCategory, setAppCategory] = useState<
    "Communication" | "Productivity" | "Design & Code" | "Custom"
  >("Productivity");
  ...
  const getIcon = (name: string) => {
    switch (name) {
      ...
      case "Mic":
        return <Mic className="w-5 h-5 brand-text-secondary" />;
      case "Layout":
        return <Layout className="w-5 h-5 brand-text-secondary" />;
      case "Code":
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-2xl p-5 border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-100">
              Executive App & Custom Runner Hub
            </h2>
            <p className="text-xs text-slate-400">
              Launch standard web tools or write/run custom executive
              micro-apps.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl brand-button font-bold text-xs flex items-center gap-1.5 brand-ring transition-all"
          >
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap ${
              selectedCategory === cat
                ? "brand-button shadow-md font-bold"
      ...
      {runningCustomApp && (
        <div className="glass-panel p-5 rounded-2xl border brand-border space-y-3">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <span className="text-xs font-bold brand-text uppercase tracking-wider flex items-center gap-1.5">
              <Terminal className="w-4 h-4" />
              <span>
                Custom App Execution Environment: {runningCustomApp.name}
              </span>
            </span>
            <button
              onClick={() => setRunningCustomApp(null)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredShortcuts.map((sc) => (
            <div
              ...
              className="glass-card p-5 rounded-2xl border border-slate-800/80 hover:border-[var(--brand-accent-border)] cursor-pointer space-y-4 group relative flex flex-col justify-between"
            >
              <div className="flex items-start justify-between">
                <div className="p-3 rounded-xl bg-obsidian-950 border border-slate-800 group-hover:border-[var(--brand-accent-border)] transition-all">
                  {getIcon(sc.icon)}
                </div>
                <button
                  onClick={(e) => {
              ...
              <div>
                <h4 className="font-bold text-slate-100 text-base group-hover:brand-text transition-colors truncate">
                  {sc.name}
                </h4>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800/60 text-xs text-slate-400">
                <span>
                  Launches:{" "}
                  <strong className="brand-text">{sc.launchCount}</strong>
                </span>
                {sc.isLocalUri ? (
                  <LocalFilePathBadge path={sc.url} />
                ) : (
                  <span className="flex items-center gap-1 brand-text group-hover:underline">
            ...
            >
              ...
              <div className="flex items-center gap-3 shrink-0 text-xs">
                {sc.isLocalUri ? (
                  ...
                  <span className="brand-text font-semibold flex items-center gap-1">
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg ${sc.pinned ? "brand-text" : "text-slate-500"}`}
                >
                  <Pin className="w-4 h-4" />
                </button>
              </div>
