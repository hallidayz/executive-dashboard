...
export const KnowledgeBaseView: React.FC<KnowledgeBaseProps> = ({
  ...
  const [category, setCategory] = useState<
    "Decision" | "Strategy" | "Influence" | "Reflection" | "Framework"
  >("Decision");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-2xl p-5 border brand-border flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="p-3 rounded-xl brand-bg-soft brand-text-secondary border brand-border">
            <Brain className="w-6 h-6" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold text-slate-100">
                Local Laptop Knowledge Base
              </h2>
            ...
            <p className="text-xs text-slate-400">
              Tracks your strategic decisions, rationales, and influence
              outcomes locally on your laptop to train your executive AI clone.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl brand-button font-bold text-xs flex items-center gap-2 brand-ring transition-all shrink-0"
        >
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto bg-obsidian-900/80 p-1.5 rounded-xl border border-slate-800 w-full md:w-auto">
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-semibold ${
              selectedTag === "All" ? "brand-button" : "text-slate-400"
            }`}
          >
            All Tags
          </button>
          {allTags.map((tag) => (
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-semibold ${
                selectedTag === tag ? "brand-button" : "text-slate-400"
              }`}
            >
              #{tag}
            </button>
      ...
      {showAddModal && (
        ...
        >
          <h4 className="font-bold brand-text-secondary text-sm">
            Log New Executive Decision & Rationale
          </h4>
      ...
      <div className="space-y-4">
        {filteredEntries.map((entry) => (
          <div
            key={entry.id}
            className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-3 hover:brand-border transition-all"
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-2">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full brand-bg-soft brand-text-secondary border brand-border text-xs font-bold">
                  {entry.category}
                </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
              ...
              <div className="p-3 rounded-xl brand-bg-soft border brand-border space-y-1">
                <span className="text-[10px] font-bold brand-text-secondary uppercase">
                  Decision Made
                </span>
                <p className="brand-text-secondary font-semibold">
                  {entry.decisionMade}
                </p>
              </div>
